...
struct Shared<Value> {
    ...
    subscript<Member: Hashable>(dynamicMember keyPath: WritableKeyPath<Storage.Value, Member?>) -> Shared<Member>? {
        if
            let member = self.wrappedValue[keyPath: keyPath] as? Member,
            let keyPath = self.keyPath.appending(path: keyPath) {
            let path = keyPath.appending(path: \Member?.[default: member])
            return Shared<Member>(self.storage, keyPath: path!)
        }
        return nil
    }
